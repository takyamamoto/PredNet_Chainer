...
class PredNet(chainer.Chain):
    def __init__(self, stack_sizes=(3, 48, 96, 192), return_Ahat=False, extrap_start_time=None):
        """
        Args:
        """
    ...
    def __call__(self, x):
        ...
        for t in range(T):
            ...
            if t == 0:
                loss_t = 0
            elif t > 0 and t < self.extrap_start_time:
                loss_t = F.average(e_t[0])
            else:
                loss_t = 0.5 * F.average(F.absolute(x[:,t]-ahat))
